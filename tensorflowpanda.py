# -*- coding: utf-8 -*-
"""
Created on Sun Dec  2 10:53:13 2018
...
import numpy as np
# ——————————————————导入数据——————————————————————
f = open('stock/600291.csv')#重新写入数据位置和名称等
df = pd.read_csv(f)  # 读入股票数据
data = df.iloc[:, 3:11].values  # 取第3-10列
print(data)
# 获取训练集，从0到5800个，即是前多少行数据
def get_train_data(batch_size=60, time_step=20, train_begin=0, train_end=100):#函数传值，
    batch_index = []
...
def get_test_data(time_step=20, test_begin=100):
...
def train_lstm(batch_size=60, time_step=20, train_begin=300, train_end=100):
